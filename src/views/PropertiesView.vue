...
import { Delete, Document, Edit, EditPen, View } from '@element-plus/icons-vue'
...
const FALLBACK_PROPERTY_STATUSES = ['草稿', '待审核', '驳回', '待开发', '出租', '已租', '出售', '已售', '待租售', '意向中', '下架封存']
...
function statusTagClass(s: string) {
  if (s === '出租' || s === '待租') return 'mint'
  if (s === '待审核') return 'amber'
  if (s === '驳回') return 'rose'
  if (s === '意向中' || s === '待租售' || s === '待开发') return 'amber'
  if (s === '草稿') return 'neutral'
  if (s === '已租' || s === '已售') return 'cyan'
  if (s === '下架封存' || s === '出售' || s === '待售') return 'rose'
  return 'mint'
}
...
function openFollow(code: string) {
  followModalCode.value = code
...
<template>
  <section class="panel active">
        ...
        <tbody>
          ...
          <tr v-for="r in list" :key="r.id" :class="{ 'property-row--featured': r.featured }">
            ...
            <td class="table-actions">
              ...
              <TableActionBtn title="写跟进" :icon="EditPen" variant="success" @click="openFollow(r.code)" />
    ...
    <PropertyFollowModal v-model:visible="followModalVisible" :code="followModalCode" />
  </section>
</template>
